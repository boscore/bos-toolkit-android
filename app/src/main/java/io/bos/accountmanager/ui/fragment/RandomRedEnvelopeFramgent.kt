...
import kotlinx.android.synthetic.main.fragment_red_envelope_random.*

/**
 * 随机红包
 * Created by Administrator on 2019/1/2/002.
 */
@Route(path = Constants.RoutePath.FRAGMENT.RED_ENVELOPE_RANDOM_FRAGMENT)
class RandomRedEnvelopeFramgent : AbstractBosFragment() {
    ...
    override fun fragmentLayout(): Int {
        return R.layout.fragment_red_envelope_random
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        select_account.setOnClickListener {
            ...
            dialogSelect?.selectAccountCallback = object : SelectAccountDialog.SelectAccountCallback {
                override fun onDetermine(select: AccountTable, positionTag: Int) {
                    this@RandomRedEnvelopeFramgent.positionTag = positionTag
        ...
        send.setOnClickListener {
            ...
            when {
                TextUtils.equals(getString(R.string.choose_pay_account), account) -> Toast.makeText(context, getString(R.string.account_not_empty), Toast.LENGTH_SHORT).show()
                TextUtils.isEmpty(amount) -> Toast.makeText(context, getString(R.string.amount_not_empty), Toast.LENGTH_SHORT).show()
                TextUtils.isEmpty(count) -> Toast.makeText(context, getString(R.string.count_not_empty), Toast.LENGTH_SHORT).show()
                amount.toDouble() < 0.1 -> Toast.makeText(context, getString(R.string.red_amount_limit), Toast.LENGTH_SHORT).show()
                count.toInt() > 100 -> Toast.makeText(context, getString(R.string.red_count_limit), Toast.LENGTH_SHORT).show()
                else -> callBack?.accountRedEnvelope(accountTable!!, amount, 2, count.toInt(), congratulations.text.toString())
            }
